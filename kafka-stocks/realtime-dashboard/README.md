...
`proxy_server.py` lives one level up in `kafka-stocks/`.

```bash
# From the repo root — Terminal 1: start the proxy
python kafka-stocks/proxy_server.py

# From the repo root — Terminal 2: serve files from kafka-stocks/
python -m http.server 8080 --directory kafka-stocks
```

Open: `http://localhost:8080/realtime-dashboard/`
